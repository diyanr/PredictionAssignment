...
1.4 Remove variables that are mostly NA:
```{r}
...
1.5 Remove the identifying variables:
```{r}
trainData <- trainData[, -seq(1,6)]
dim(trainData)
```

1.6 Partition the training data into a training set and a test set:
```{r}
...
2.1 Linear Distriminant Analysis
...
3. RUN THE BEST MODEL ON THE TEST DATA
